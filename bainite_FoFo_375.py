...
aus1 = FCC(T_C=T_C, dt=dt, z=np.linspace(-1.16, -.58, 100), c0=c0,
           tdata=tdata_fcc)
fer2 = BCC(T_C=T_C, dt=dt, z=np.linspace(-.58, -.58, 10), c0=0.,
           tdata=tdata_bcc, E=WBs(T_C))
aus2 = FCC(T_C=T_C, dt=dt, z=np.linspace(-.58, 0, 100), c0=c0,
           tdata=tdata_fcc)
fer3 = BCC(T_C=T_C, dt=dt, z=np.linspace(0, 0, 10), c0=0.,
           tdata=tdata_bcc, E=WBs(T_C))
...
int4 = Interface(domain1=aus2, domain2=fer3, type_int='mobile.mmode')
...
for i in control_itsteps.itlist:
    ...
    int4.v = 1e6*int4.chem_driving_force()*int4.M()/fer3.Vm
