...
os.system('rm C_profiles/{}*'.format(basename))
...
mart = BCC(T_C=T_C, dt=dt, z=np.linspace(-1.16, -.66, 40), c0=c0,
...
muC = 30e3
cCCEtheta = aus1.mu2x['C'](muC)
int1.ci_fcc = cCCEtheta
print('muC={:}, ci_fcc={:}\n'.format(muC, int1.ci_bcc))
...
for i in range(n_time):
    try:
        # boundary conditions do not change whether fer1_diss is True or False
        J, = int1.flux('fcc')
        if mart.FDM_implicit(bcn=(1.5, -2., .5, -J*mart.dz/mart.D()), lowerbound=5.4e-4):
        # It should be:
        # if mart.FDM_implicit(bcn=(1.5, -2., .5, -J*mart.dz/mart.D(mart.c[-1])), lowerbound=5.4e-4):
        # But it the program fails after a couple of iterations. 
        # Anyway the effect is very small, because the system rapidly
        # changes from CCEtheta to CCE
